...
#Поэтому вы должны постепенно увеличивать SECURE_HSTS_SECONDS, проверяя, что ничто не ломается.
#Делайте это в течение нескольких дней, проверяя обратную связь с вашими пользователями.
#Начните с чего-то небольшого, например, 30 секунд, и добавляйте до 31536000 секунд (то есть 1 год).
SECURE_HSTS_SECONDS = 3600

# Если установлено значение True , cookie будет помечен как «безопасный», что означает, что браузеры несут
# ответственность за проверку того, что cookie отправляется только для HTTPS-соединений.
# На самом деле не рекомендуется оставлять этот параметр отключенным, поскольку злоумышленник может
# захватить незашифрованный файл cookie сеанса с помощью анализатора пакетов, а затем использовать этот файл cookie для взлома сеанса пользователя.
...
# Рекомендуем включить эту настройку (убедитесь, что все поддомены работают по HTTPS),
SECURE_HSTS_INCLUDE_SUBDOMAINS = True

# При True SecurityMiddleware перенаправляет все не-HTTPS запросы на HTTPS (кроме тех URL-ов, которые удовлетворяют
# регулярному выржению из SECURE_REDIRECT_EXEMPT).
