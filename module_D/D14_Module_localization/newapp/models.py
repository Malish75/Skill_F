...
# from django.utils.translation import ugettext_lazy as _# импортируем «ленивый» геттекст с подсказкой
...
class Category(models.Model):
    category_name = models.CharField(max_length=30, unique=True, null=False, help_text=_('название категории'))  # переводящийся текст подсказку к полю
...
class Post(models.Model):
    ...
    def save(self, *args, **kwargs):                 # переопределение для обновления ключей кеша конкретной новости (смотри PostDetailView)
        super().save(*args, **kwargs)                # сначала вызываем метод родителя, чтобы объект сохранился
        cache.delete(f'new_post-{self.pk}')          # затем удаляем его из кэша, чтобы сбросить его
